...
## 14
- イテレータ
  - イテレータを実行するために、Iterの型に変換させないといけない。
  - `let i = vec![1,2,3]; i.iter();` => イテレータのstructになる。
  - Iteratorトレイト
    - 消費アダプタ（consuming adaptors）：Iteratorトレイトに実装されてる、消費系のメソッドを呼び出す。
      - next：mutで値を次々に移動させる。消費される。
      - collect: mapなどのイテレータを解決させる
    - イテレータアダプタ（iterator adaptors）：Iteratorトレイトに実装されてる、別のイテレータに変換するメソッド。
  - 消費とイテレータ
